...
using namespace RooFit;
  ...
  RooDataSet* data_p=new RooDataSet("data_p","data_p",RooArgSet(delmass,w));
  RooDataSet* data_n=new RooDataSet("data_n","data_n",RooArgSet(delmass,w));
...
if(photon1cutflag == 1 && photon1cutflag == 1){
	      ...
	      if(f_dstcharge==+1)
		{
		  nevt_ac_p++;
w.setVal(dpw[bin]);
data_p->add(RooArgSet(delmass,w));
	      ...
	      else
		{
		  nevt_ac_n++;
w.setVal(dnw[bin]);
data_n->add(RooArgSet(delmass,w));
...
//RooProdPdf Bkg_P("Bkg_P","Bkg_P",RooArgSet(bkg_p,bkgH_p));
  RooAddPdf Model_P("Model_P", "Model_P",RooArgList(sig_tot_p,bkg_p), RooArgList(N_p,N_pb));
...
//RooProdPdf Bkg_N("Bkg_N","Bkg_N",RooArgSet(bkg_n,bkgH_n));
  RooAddPdf Model_N("Model_N", "Model_N",RooArgList(sig_tot_n,bkg_n), RooArgList(N_n,N_nb));
  ...
  RooDataSet* combData =new RooDataSet("combData","combData",RooArgSet(delmass,w),Index(sample),Import("pos",*data_p),Import("neg",*data_n));
