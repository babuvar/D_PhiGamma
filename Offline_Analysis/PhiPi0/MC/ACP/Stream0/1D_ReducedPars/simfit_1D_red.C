...
Int_t maxp=data_p->numEntries(),maxn=data_n->numEntries();
  ...
  //CHARGE=+1
  RooRealVar fudge("fudge","fudge",1.0,0.0,3.0);

  RooRealVar mean("mean","mean",0.145428,0.14,0.15);
  RooRealVar sig_fixed("sig_fixed","sig",sigma);
  RooFormulaVar sig("sig","sig_fixed*fudge",RooArgList(sig_fixed,fudge));;//**
  RooGaussian sig_pdf_p("sig_pdf_p", "signal Gaussian",delmass,mean,sig);

  RooRealVar sig1_fixed("sig1_fixed","sig1",sigma1);
  RooRealVar sig2_fixed("sig2_fixed","sig2",sigma2);
  RooFormulaVar sig1("sig1","sig1_fixed*fudge",RooArgList(sig1_fixed,fudge));//**
  RooFormulaVar sig2("sig2","sig2_fixed*fudge",RooArgList(sig2_fixed,fudge));//**
  ...
  RooRealVar f_Sig_fixed("f_Sig_fixed","f_Sig",fraction);
  RooFormulaVar f_Sig("f_Sig","f_Sig_fixed*fudge",RooArgList(f_Sig_fixed,fudge));//**
