...
//  RooRealVar helicity("cos(#theta_{hel})","helicity",-1,1,"no unit");
  ...
  RooDataSet* data_p=new RooDataSet("data_p","data_p",RooArgSet(helicity,delmass,dzero,w));
  RooDataSet* data_n=new RooDataSet("data_n","data_n",RooArgSet(helicity,delmass,dzero,w));
...
void fit3d_w(void)
{
  ...
  chain->Add("PhiGamma_1s_data.root");
  chain->Add("PhiGamma_2s_data.root");
  chain->Add("PhiGamma_3s_data.root");
  chain->Add("PhiGamma_4s_continuum_data.root");
 ...
 Float_t Deltam, Dzeromas, Dstarmas, Mygenfla, Dstarcha, Dstp, f_helicity, Gamener, P_piz, P_eta, Vetom1, Vetom2, ProbPi, ProbEta, MPhi, MD0 ,  E9E25, Piflag, Mypgenfl, f_Kppid,f_Kmpid,f_Pispid,f_Kpdr,f_Kmdr,f_Pisdr,f_Kpdz,f_Kmdz,f_Pisdz,f_helicity2, f_dstcts;
  ...
  h1->SetBranchAddress("Dstcts",&f_dstcts);
...
if(Mygenfla==1){numsig++;}
		...
		{		  helicity.setVal(f_helicity);
		  nevt_ac_p++;w.setVal(dpw[bin]);
data_p->add(RooArgSet(helicity,delmass,dzero,w));
		}
	      else if(Dstarcha==-1)
		{		  helicity.setVal(-f_helicity);
		  nevt_ac_n++;w.setVal(dnw[bin]);
data_n->add(RooArgSet(helicity,delmass,dzero,w));
		}
}}}}}}}}}}}}
    }
  ...
  for(int i=nevt;i<nevt1;i++) 
    ...
    {
      chain->GetEntry(i); bin=floor(f_dstcts/mnum)+num;
		  delmass.setVal(Deltam);
		  dzero.setVal(MD0);
...
//Background2 Helicity
  RooGenericPdf bkg2H_p("bkg2H_p","(helicity-shift2)*(helicity-shift2)",RooArgSet(helicity,shift2));
...
//Background2 Helicity
  RooGenericPdf bkg2H_n("bkg2H_n","(helicity-shift2)*(helicity-shift2)",RooArgSet(helicity,shift2));
...
//Full Model N
  ...
  RooDataSet* combData =new RooDataSet("combData","combData",RooArgSet(helicity,delmass,dzero,w),Index(sample),Import("pos",*data_p),Import("neg",*data_n));
...
//  RooPlot *xframe_1 =delmass.frame(Bins(50),Cut("dzero >= 1.84 && dzero <= 1.89"),Title("D^{0} #rightarrow #phi #gamma"));
  ...
  simPdf.plotOn(xframe_1,Slice(sample,"pos"),Components("sig_tot_p"),ProjWData(sample,*combData_w),LineStyle(kDashed));
  simPdf.plotOn(xframe_1,Slice(sample,"pos"),Components("bkg_p"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kGreen));
  simPdf.plotOn(xframe_1,Slice(sample,"pos"),Components("bkg_tot2_p"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kRed));
...
//  RooPlot *xframe_2 = delmass.frame(Bins(50),Title("D^{0} #rightarrow #phi #gamma"));
  ...
  simPdf.plotOn(xframe_2,Slice(sample,"neg"),Components("sig_tot_n"),ProjWData(sample,*combData_w),LineStyle(kDashed));
  simPdf.plotOn(xframe_2,Slice(sample,"neg"),Components("bkg_n"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kGreen));
  simPdf.plotOn(xframe_2,Slice(sample,"neg"),Components("bkg_tot2_n"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kRed));
 ...
 //    Model_N.paramOn(xframe_2,data_n);  
  ...
  simPdf.plotOn(yframe_1,Slice(sample,"pos"),Components("sig_p"),ProjWData(sample,*combData_w),LineStyle(kDashed));
  simPdf.plotOn(yframe_1,Slice(sample,"pos"),Components("bkgH1_p"),ProjWData(sample,*combData_w),LineStyle(kDashed), LineColor(kGreen));
  simPdf.plotOn(yframe_1,Slice(sample,"pos"),Components("bkg2H_p"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kRed));
...
//   Model_P.paramOn(yframe_1,data_p);
  ...
  simPdf.plotOn(yframe_2,Slice(sample,"neg"),Components("sig_n"),ProjWData(sample,*combData_w),LineStyle(kDashed));
  simPdf.plotOn(yframe_2,Slice(sample,"neg"),Components("bkgH1_n"),ProjWData(sample,*combData_w), LineStyle(kDashed),LineColor(kGreen));
  simPdf.plotOn(yframe_2,Slice(sample,"neg"),Components("bkg2H_n"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kRed));
...
sample.setRange("good","pos") ;
  ...
  combData_w.plotOn(zframe_1,Cut("sample==sample::pos"));


  simPdf.plotOn(zframe_1,Slice(sample,"pos"),ProjWData(sample,*combData_w));
  simPdf.plotOn(zframe_1,Slice(sample,"pos"),Components("sig_dz_p"),ProjWData(sample,*combData_w),LineStyle(kDashed));
  simPdf.plotOn(zframe_1,Slice(sample,"pos"),Components("Bkg_D0_p"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kGreen));
  simPdf.plotOn(zframe_1,Slice(sample,"pos"),Components("Bkg_D0_2p"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kRed));
...
//   Model_P.paramOn(yframe_1,data_p);
  ...
  combData_w.plotOn(zframe_2,Cut("sample==sample::neg"));
  simPdf.plotOn(zframe_2,Slice(sample,"neg"),ProjWData(sample,*combData_w));
  simPdf.plotOn(zframe_2,Slice(sample,"neg"),Components("sig_dz_n"),ProjWData(sample,*combData_w),LineStyle(kDashed));
  simPdf.plotOn(zframe_2,Slice(sample,"neg"),Components("Bkg_D0_n"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kGreen));
  simPdf.plotOn(zframe_2,Slice(sample,"neg"),Components("Bkg_D0_2n"),ProjWData(sample,*combData_w),LineStyle(kDashed),LineColor(kRed));
//     Model_N.paramOn(yframe_2,data_n);  
//  TCanvas* c2 = new TCanvas("c2","c2",1200,800) ;
