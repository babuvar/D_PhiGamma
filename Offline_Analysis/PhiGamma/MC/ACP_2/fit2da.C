...
void fit2da(void)
{
  ...
  chain->Add("PhiGamma_4s_FullMC_0.root");
...
data_n->add(RooArgSet(helicity,delmass,dzero));
  ...
  chain->Add("PhiGamma_5s_FullMC_0.root");
...
data_n->add(RooArgSet(helicity,delmass,dzero));
  ...
  RooRealVar sig1("sig1","sig1",0.000841);//**
  RooRealVar sig2("sig2","sig2",0.001012);//**
  RooRealVar sig1b("sig1b","sig1b",0.000352);//**
  RooRealVar sig2b("sig2b","sig2b",0.000510);//**
  RooRealVar f_Sig("f_Sig","f_Sig",0.765);//**
  RooRealVar f_Bkg2("f_Bkg2","f_Bkg2",0.252);//**
  ...
  RooRealVar m("m","m",1.86494);
  RooRealVar s("s","s",0.011488);
  RooRealVar a("a","a",0.728);
  RooRealVar n("n","n",6.42);
  RooRealVar p1("p1","p1",0.0);   
  RooRealVar c1("param_{expo}","c1",-9,-12,0);
  RooRealVar f1("frac_{cheby/expo}","f1",0.5,0,1);
   RooRealVar m1("m1","m1",1.84686);
  RooRealVar s1("s1","s1",0.01336);
  RooRealVar A1("A1","A1",0.1605);
  RooRealVar n1("n1","n1",93.07);
...
//Product Signal
RooProdPdf Sig_P("Sig_P","Sig_P",RooArgSet(sig_tot_p,sig_p));
...
//Product Background1
RooProdPdf Bkg1_P("Bkg1_P","Bkg1_P",RooArgSet(bkg_p,bkgH1_p));
...
//Background2 DeltaM
  ...
  //Double-Gauss
  RooBifurGauss bkg_pdf1_p("bkg_pdf1_p", "signal Gaussian",delmass,mean,sig1b,sig2b);
...
//Product Background2
RooProdPdf Bkg2_P("Bkg2_P","Bkg2_P",RooArgSet(bkg_tot2_p,bkg2H_p));
...
//Product Signal
RooProdPdf Sig_N("Sig_N","Sig_N",RooArgSet(sig_tot_n,sig_n));
...
//Product Background1
RooProdPdf Bkg1_N("Bkg1_N","Bkg1_N",RooArgSet(bkg_n,bkgH1_n));
...
//Background2 DeltaM
  ...
  //Double-Gauss
  RooBifurGauss bkg_pdf1_n("bkg_pdf1_n", "signal Gaussian",delmass,mean,sig1b,sig2b);
...
//Product Background2
RooProdPdf Bkg2_N("Bkg2_N","Bkg2_N",RooArgSet(bkg_tot2_n,bkg2H_n));
