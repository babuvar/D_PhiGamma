...
TGraph2D *g = new TGraph2D();
	g->SetTitle("2-D Optimization for #Upsilon(4S)");
...
for(int k=1;k<=120;k++){    // for E(Gam)
for(int k2=1;k2<=200;k2++){ //for P*(D*)
cut = (float)k*0.01+0.2;  //200 MeV
cut2 = (float)k2*0.01+2.0;  //2 GeV
...
for(int k=1;k<=120;k++){    
for(int k2=1;k2<=200;k2++){ 
//fom[k][k2]=(float)sqrt(nsignal[k][k2]+nback[k][k2])/nsignal[k][k2];
fom[k][k2]=(float)sqrt((nsignal[k][k2]/20.0)+nback[k][k2])/(nsignal[k][k2]/20.0);
x[k][k2]=(float)k*10+200;
y[k][k2]=(float)k2*0.01+2.0;
g->SetPoint(count,x[k][k2],y[k][k2],fom[k][k2]);
count++;
if(fom[k][k2] <= bestfom){bestfom=fom[k][k2];bestk=k; bestk2=k2;}
}}

cout<<fom[bestk][bestk2]<<"\t"<<nsignal[bestk][bestk2]/20.0<<"\t"<<nback[bestk][bestk2]/20.0<<"\t best E(Gam) = "<<(float)bestk*10+200<<"MeV\t best P*(D*) = "<<(float)bestk2*0.01+2.0<<"GeV"<<endl;
