void MD(){
  ...
  TFile *f = new TFile("FullPhiGammaMC.root");
...
if(ProbEta < 0.285){
if(E9E25 > 0.947){
if(Gamener > 0.59 && Dstp > 3.1){//Satisfies Pi0 Veto
...
fom[k][k2]=(float)sqrt((nsignal[k][k2]/200.0)+(nback[k][k2]/6.0))/(nsignal[k][k2]/200.0);
...
cout<<fom[bestk][bestk2]<<"\t"<<nsignal[bestk][bestk2]/200.0<<"\t"<<nback[bestk][bestk2]<<"\t upper cut = "<<cut<<"MeV\t lower cut = "<<cut2<<"GeV"<<endl;
...
//0.197433	53.32	345	 upper cut = 1.89MeV	 lower cut = 1.84GeV
