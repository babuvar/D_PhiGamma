void PiVeto(){
  ...
  int nsignal[401]={0},nback[401]={0},bestk;
  float veto,fom[401],bestfom=100;
...
for(int k=1;k<=400;k++){
veto = (float)k*0.001;
if(ProbPi < veto){//Satisfies Pi0 Veto
...
for(int k=1;k<=400;k++){
fom[k]=(float)sqrt((nsignal[k]/100.0)+(nback[k]/6.0))/(nsignal[k]/100.0);
//cout<<fom[k]<<"\t"<<nsignal[k]<<endl;
if(fom[k] <= bestfom){bestfom=fom[k];bestk=k;}
}

cout<<fom[bestk]<<"\t"<<(nsignal[bestk]/100.0)<<"\t best veto = "<<(float)bestk*0.001<<endl;
...
//0.312347	138.4	 best veto = 0.056
