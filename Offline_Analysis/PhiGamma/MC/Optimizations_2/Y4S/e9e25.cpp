...
if(nsignal[k] != 0){fom[k]=(float)sqrt((nsignal[k]/10.0)+nback[k])/(nsignal[k]/10.0);}else{fom[k]==1000;}
cout<<"fom = "<<fom[k]<<endl;
if(fom[k] <= bestfom){bestfom=fom[k];bestk=k;}
}

cout<<fom[bestk]<<"\t"<<nsignal[bestk]/10<<"\t best veto = "<<(float)bestk*0.001+0.9<<endl;
