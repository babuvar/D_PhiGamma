...
void fit3d_proj(void)
{
  //LOAD DATA FILE
  TChain* chain=new TChain("h1");

  chain->Add("PhiGamma_4s_FullMC.root");
...
data_n->add(RooArgSet(helicity,delmass,dzero));
  ...
  chain->Add("PhiGamma_5s_FullMC.root");
...
data_n->add(RooArgSet(helicity,delmass,dzero));
  ...
  RooRealVar sig1("sig1","sig1",0.000966);//**
  RooRealVar sig2("sig2","sig2",0.001156);//**
  RooRealVar sig1b("sig1b","sig1b",0.001213);//**
  RooRealVar sig2b("sig2b","sig2b",0.001492);//**
  RooRealVar f_Sig("f_Sig","f_Sig",0.7317);//**
  RooRealVar f_Bkg2("f_Bkg2","f_Bkg2",0.364);//**
  ...
  RooRealVar m("m","m",1.864779);//**
  RooRealVar s("s","s",0.011590);//**
  RooRealVar a("a","a",0.738);//**
  RooRealVar n("n","n",6.28);//**
  RooRealVar p1("p1","p1",0.0);   
  RooRealVar c1("param_{expo}","c1",-9,-12,0);
  RooRealVar f1("frac_{cheby/expo}","f1",0.5,0,1);
   RooRealVar m1("m1","m1",1.84624);//**
  RooRealVar s1("s1","s1",0.01377);//**
  RooRealVar A1("A1","A1",0.161);//**
  RooRealVar n1("n1","n1",100);//**
...
//Defining signal windows
 ...
 dzero.setRange("RectSignal",1.83,1.89);	//DeltaM window 'signal2'
  ...
  //DeltaM PLOTING
//  RooPlot *xframe_1 =delmass.frame(Bins(50),Cut("dzero >= 1.84 && dzero <= 1.89"),Title("D^{0} #rightarrow #phi #gamma"));
  RooPlot *xframe_1 =delmass.frame(Bins(50),Title("D^{0} #rightarrow #phi #gamma"));
  combData.plotOn(xframe_1,Cut("sample==sample::pos"),CutRange("SignalforDelM"));
  simPdf.plotOn(xframe_1,Slice(sample,"pos"),ProjWData(sample,combData),ProjectionRange("SignalforDelM"));
  simPdf.plotOn(xframe_1,Slice(sample,"pos"),Components("sig_tot_p"),ProjWData(sample,combData),LineStyle(kDashed),ProjectionRange("SignalforDelM"));
  simPdf.plotOn(xframe_1,Slice(sample,"pos"),Components("bkg_p"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kGreen),ProjectionRange("SignalforDelM"));
  simPdf.plotOn(xframe_1,Slice(sample,"pos"),Components("bkg_tot2_p"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kRed),ProjectionRange("SignalforDelM"));
   Model_P.paramOn(xframe_1,data_p);
//  RooPlot *xframe_2 = delmass.frame(Bins(50),Cut("dzero <= 1.84 && dzero >= 1.89"),Title("D^{0} #rightarrow #phi #gamma"));
  RooPlot *xframe_2 = delmass.frame(Bins(50),Title("D^{0} #rightarrow #phi #gamma"));
//  RooPlot *xframe_2 = delmass.frame(Bins(50),Title("D^{0} #rightarrow #phi #gamma"));
  combData.plotOn(xframe_2,Cut("sample==sample::neg"),CutRange("SignalforDelM"));
  simPdf.plotOn(xframe_2,Slice(sample,"neg"),ProjWData(sample,combData),ProjectionRange("SignalforDelM"));
  simPdf.plotOn(xframe_2,Slice(sample,"neg"),Components("sig_tot_n"),ProjWData(sample,combData),LineStyle(kDashed),ProjectionRange("SignalforDelM"));
  simPdf.plotOn(xframe_2,Slice(sample,"neg"),Components("bkg_n"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kGreen),ProjectionRange("SignalforDelM"));
  simPdf.plotOn(xframe_2,Slice(sample,"neg"),Components("bkg_tot2_n"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kRed),ProjectionRange("SignalforDelM"));
  ...
  combData.plotOn(yframe_1,Cut("sample==sample::pos"),CutRange("RectSignal"));
  simPdf.plotOn(yframe_1,Slice(sample,"pos"),ProjWData(sample,combData), ProjectionRange("RectSignal"));
  simPdf.plotOn(yframe_1,Slice(sample,"pos"),Components("sig_p"),ProjWData(sample,combData),LineStyle(kDashed),ProjectionRange("RectSignal"));
  simPdf.plotOn(yframe_1,Slice(sample,"pos"),Components("bkgH1_p"),ProjWData(sample,combData),LineStyle(kDashed), LineColor(kGreen),ProjectionRange("RectSignal"));
  simPdf.plotOn(yframe_1,Slice(sample,"pos"),Components("bkg2H_p"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kRed),ProjectionRange("RectSignal"));

//   Model_P.paramOn(yframe_1,data_p);

  RooPlot *yframe_2 = helicity.frame(Bins(50),Title("D^{0} #rightarrow #phi #gamma"));
  combData.plotOn(yframe_2,Cut("sample==sample::neg"),CutRange("RectSignal"));
  simPdf.plotOn(yframe_2,Slice(sample,"neg"),ProjWData(sample,combData),ProjectionRange("RectSignal"));
  simPdf.plotOn(yframe_2,Slice(sample,"neg"),Components("sig_n"),ProjWData(sample,combData),LineStyle(kDashed),ProjectionRange("RectSignal"));
  simPdf.plotOn(yframe_2,Slice(sample,"neg"),Components("bkgH1_n"),ProjWData(sample,combData), LineStyle(kDashed),LineColor(kGreen),ProjectionRange("RectSignal"));
  simPdf.plotOn(yframe_2,Slice(sample,"neg"),Components("bkg2H_n"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kRed),ProjectionRange("RectSignal"));
...
sample.setRange("good","pos") ;

  //D0 PLOTING
  RooPlot *zframe_1 =dzero.frame(Bins(50),Title("D^{0} #rightarrow #phi #gamma"));
//  combData.plotOn(zframe_1,Cut("sample==sample::pos && delmass > 0.15"));
  combData.plotOn(zframe_1,Cut("sample==sample::pos"),CutRange("SignalforMD"));
//  combData.plotOn(zframe_1,Cut("good"));

  simPdf.plotOn(zframe_1,Slice(sample,"pos"),ProjWData(sample,combData),ProjectionRange("SignalforMD"));
  simPdf.plotOn(zframe_1,Slice(sample,"pos"),Components("sig_dz_p"),ProjWData(sample,combData),LineStyle(kDashed),ProjectionRange("SignalforMD"));
  simPdf.plotOn(zframe_1,Slice(sample,"pos"),Components("Bkg_D0_p"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kGreen),ProjectionRange("SignalforMD"));
  simPdf.plotOn(zframe_1,Slice(sample,"pos"),Components("Bkg_D0_2p"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kRed),ProjectionRange("SignalforMD"));
...
//   Model_P.paramOn(yframe_1,data_p);
  ...
  combData.plotOn(zframe_2,Cut("sample==sample::neg"),CutRange("SignalforMD"));
  simPdf.plotOn(zframe_2,Slice(sample,"neg"),ProjWData(sample,combData),ProjectionRange("SignalforMD"));
  simPdf.plotOn(zframe_2,Slice(sample,"neg"),Components("sig_dz_n"),ProjWData(sample,combData),LineStyle(kDashed),ProjectionRange("SignalforMD"));
  simPdf.plotOn(zframe_2,Slice(sample,"neg"),Components("Bkg_D0_n"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kGreen),ProjectionRange("SignalforMD"));
  simPdf.plotOn(zframe_2,Slice(sample,"neg"),Components("Bkg_D0_2n"),ProjWData(sample,combData),LineStyle(kDashed),LineColor(kRed),ProjectionRange("SignalforMD"));
//     Model_N.paramOn(yframe_2,data_n);  
//  TCanvas* c2 = new TCanvas("c2","c2",1200,800) ;
...
//  c2->Divide(2) ;
  ...
  c->SaveAs("img.png");  delete c;
  c2->SaveAs("img2.png");   delete c2;
  c3->SaveAs("img3.png");   delete c3;
  c4->SaveAs("img4.png");   delete c4;
