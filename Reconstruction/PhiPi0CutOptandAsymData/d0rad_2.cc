...
nt_check = BASF_Histogram->ntuple( title.c_str(),"Kprftheta Kprfphi D0rftheta D0rfphi DstTheta DstPhi DzTheta DzPhi PisTheta PisPhi PhiTheta PhiPhi PizTheta PizPhi KpTheta KpPhi KmTheta KmPhi Event BlueEvent Multiplicity PMultiplicity NMultiplicity PiSCT KID PiZMassNmc KMinHelicity KPlusHelicity ThetaHel PiZMomentum PisMom PisCosT Photon1Energy Photon2Energy PhiMass DZeroMass DStarCharge DStarMass DeltaM DSTID  D0ID  PiSID PhiID Pi0ID KpID KmID DSTF D0F  PiSF PhiF Pi0F KpF KmF MyGenFlag GenFlag2  DSTPS DSTCTS DR DZ", 1);
...
//Theta
      nt->column("DstTheta",Dst_cand.p().vect().theta());
      nt->column("DzTheta",Dst_cand.child(0).p().vect().theta());
      nt->column("PisTheta",Dst_cand.child(1).p().vect().theta());
      nt->column("PhiTheta",Dst_cand.child(0).child(0).p().vect().theta());
      nt->column("PizTheta",Dst_cand.child(0).child(1).p().vect().theta());
      nt->column("KpTheta",Dst_cand.child(0).child(0).child(0).p().vect().theta());
      nt->column("KmTheta",Dst_cand.child(0).child(0).child(1).p().vect().theta());
//Phi
      nt->column("DstPhi",Dst_cand.p().vect().phi());
      nt->column("DzPhi",Dst_cand.child(0).p().vect().phi());
      nt->column("PisPhi",Dst_cand.child(1).p().vect().phi());
      nt->column("PhiPhi",Dst_cand.child(0).child(0).p().vect().phi());
      nt->column("PizPhi",Dst_cand.child(0).child(1).p().vect().phi());
      nt->column("KpPhi",Dst_cand.child(0).child(0).child(0).p().vect().phi());
      nt->column("KmPhi",Dst_cand.child(0).child(0).child(1).p().vect().phi());

//"DstTheta DstPhi DzTheta DzPhi PisTheta PisPhi PhiTheta PhiPhi PizTheta PizPhi KpTheta KpPhi KmTheta KmPhi
...
//Rest Frame Directions (Trying to look for lopsidedness)
      nt->column("Kprftheta",kp4V.vect().theta());
      nt->column("Kprfphi",kp4V.vect().phi());
      nt->column("D0rftheta",dzero4V.vect().theta());
      nt->column("D0rfphi",dzero4V.vect().phi());
