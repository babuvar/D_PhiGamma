...
int A[100]={0},B[100]={0},n=0,C[100]={0},D[100]={0},nK=0,count1=0;
...
nt_check = BASF_Histogram->ntuple( title.c_str(),"BremFlag KID PiZMassNmc KPlusHelicity PiZMomentum PisMom PisCosT Photon1Energy Photon2Energy PhiMass DZeroMass DStarCharge DStarMass DeltaM DSTID  D0ID  PiSID PhiID Pi0ID KpID KmID DSTF D0F  PiSF PhiF Pi0F KpF KmF MyGenFlag GenFlag2  DSTPS DSTCTS DR DZ", 1);
...
HepLorentzVector DSTPPS,pizero4V,gamma4V,dzero4V,phi4V,kp4V,pis4v;
Hep3Vector pizero_boost,phi_boost;

double massdiff,kid,dstpps,dstcts,cos_helicity,cos_helicity2,pis3vmag,pis3vcost;
...
//My Gen Logic
int mygenflag=0,genflag2=0,allflags=0,bremflag=0;
...
//Extra Brem FSP for phi decay
if((DstGen.daLast()-DstGen.daFirst())==1 && (DzGen.daLast()-DzGen.daFirst())==1 && (PhiGen.daLast()-PhiGen.daFirst())==2 && (PizGen.daLast()-PizGen.daFirst())==1){
bremflag=1;
}//Extra Brem FSP for phi decay
